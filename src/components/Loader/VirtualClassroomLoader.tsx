...
// We keep the original vibrant colors for the 3D objects as they look great, 
// but we will update the HTML/CSS around it to match NexLearn's theme.
  ...
  return (
    <>
      ...
      <mesh position={[0, -3.2, -0.9]} rotation={[-Math.PI / 2, 0, 0]}>
        ...
        <meshBasicMaterial color="#d9f0ff" transparent opacity={0.15} />
      </mesh>
...
interface VirtualClassroomLoaderProps {
  onComplete?: () => void;
}

export function VirtualClassroomLoader({ onComplete }: VirtualClassroomLoaderProps) {
  const [progress, setProgress] = useState(0);

  useEffect(() => {
    const interval = window.setInterval(() => {
      setProgress((current) => {
        const next = current + Math.round(Math.random() * 15 + 10);
        if (next >= 100) {
          window.clearInterval(interval);
          if (onComplete) setTimeout(onComplete, 500); // Wait a brief moment at 100% before completing
          return 100;
        }
        return next;
      });
    }, 400); // Sped up the simulation slightly
    return () => window.clearInterval(interval);
  }, [onComplete]);

  return (
    <div className="loader-shell fixed inset-0 z-[9999] bg-background text-on-surface">
      <section className="stage bg-surface" aria-label="Interactive virtual classroom loading screen">
        ...
        <div className="hud">
          <div className="brand-row">
            ...
            <span className="text-primary font-bold">Scholar Hub</span>
          </div>
          <h1 className="text-on-surface">Preparing your session</h1>
          <div className="progress-wrap">
            <div className="progress-bar" style={{ width: `${progress}%` }} />
          </div>
          <div className="status-row">
            <span className="text-on-surface-variant">{progress}% loaded</span>
          </div>
        </div>

        <div className="classroom-panel bg-surface-container border-outline-variant">
          <div className="panel-top border-outline-variant">
            <span className="bg-error" />
            <span className="bg-secondary" />
            <span className="bg-primary" />
          </div>
          <div className="lesson-lines">
            <span className="bg-gradient-to-r from-primary to-secondary opacity-50" />
            <span className="bg-gradient-to-r from-primary to-secondary opacity-50" />
            <span className="bg-gradient-to-r from-primary to-secondary opacity-50" />
          </div>
        </div>
