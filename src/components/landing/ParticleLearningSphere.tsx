      ...
      <shaderMaterial
        ...
        fragmentShader={`
          ...
          void main() {
            ...
            gl_FragColor = vec4(color, falloff * (0.52 + vPulse * 0.68) * uPerformance);
          }
        `}
      />
    </points>
        ...
        return (
          <group key={item.label} position={[x, y, z]}>
            ...
            <Html center distanceFactor={8} transform sprite zIndexRange={[20, 0]}>
              <div className="pointer-events-none select-none whitespace-nowrap rounded-full border border-primary/30 bg-surface/80 px-3 py-1.5 text-[11px] font-semibold tracking-normal text-on-surface shadow-[0_8px_28px_rgba(109,93,252,0.30)] backdrop-blur-md sm:text-xs">
...
function Scene({
  ...
  return (
    <>
      {/* No <color> attach="background" — canvas is transparent */}
...
export default function ParticleLearningSphere({
  ...
  return (
    ...
    >
      {/* Soft radial glow that blends into the page background */}
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(109,93,252,0.18),transparent_65%)]" />
      <Canvas
        dpr={dpr}
        camera={{ position: [0, 0, 6.4], fov: 42, near: 0.1, far: 100 }}
        gl={{
          alpha: true,          // transparent canvas — no background override
          antialias: dpr <= 1,
      ...
      <div className="pointer-events-none absolute inset-x-6 bottom-3 h-px bg-gradient-to-r from-transparent via-primary/40 to-transparent" />
    </div>
  );
}
