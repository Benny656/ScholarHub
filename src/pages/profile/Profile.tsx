...
export function Profile() {
  ...
  const inputStyle = 'w-full px-4 py-2.5 rounded-xl border border-white/10 focus:border-purple-500/60 text-white text-sm outline-none transition-all placeholder-slate-600';
  const style = { background: 'rgba(255,255,255,0.05)', fontFamily: 'Inter, sans-serif' };
      ...
      <div className="p-6 max-w-3xl mx-auto space-y-5">
          ...
          <div className="flex items-start gap-5">
            <div className="relative flex-shrink-0">
              <motion.div
                ...
                className="w-24 h-24 rounded-2xl bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center text-3xl font-black text-white cursor-pointer"
              ...
              <button
                onClick={handleAvatarUpload}
                className="absolute -bottom-1 -right-1 w-7 h-7 rounded-xl flex items-center justify-center text-white transition-all"
            ...
            <div className="flex-1">
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="text-xl font-bold text-white mb-1" style={{ fontFamily: 'Geist, sans-serif' }}>{form.name}</h2>
                  <p className="text-sm text-slate-400 mb-2" style={{ fontFamily: 'Inter, sans-serif' }}>{form.email}</p>
        ...
        <div className="flex gap-1 p-1 rounded-2xl" style={{ background: 'rgba(255,255,255,0.04)' }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id as any)}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl text-sm font-medium transition-all ${tab === t.id ? 'text-white' : 'text-slate-400 hover:text-white'}`}
        ...
        {tab === 'profile' && (
          <motion.div key="profile" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <GlassCard>
              <h2 className="text-base font-bold text-white mb-4" style={{ fontFamily: 'Geist, sans-serif' }}>Personal Information</h2>
        ...
        {tab === 'security' && (
          <motion.div key="security" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            <GlassCard>
              <h2 className="text-base font-bold text-white mb-4" style={{ fontFamily: 'Geist, sans-serif' }}>Change Password</h2>
            ...
            <GlassCard>
              <h2 className="text-base font-bold text-white mb-4" style={{ fontFamily: 'Geist, sans-serif' }}>Two-Factor Authentication</h2>
              <div className="flex items-center justify-between p-4 rounded-xl border border-white/8" style={{ background: 'rgba(255,255,255,0.04)' }}>
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${twoFAEnabled ? 'bg-emerald-500/20' : 'bg-white/8'}`}>
                    <Smartphone size={18} className={twoFAEnabled ? 'text-emerald-400' : 'text-slate-400'} />
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-white">Authenticator App</p>
                    <p className="text-xs text-slate-400">{twoFAEnabled ? 'Enabled — Using TOTP app' : 'Not enabled — Recommended'}</p>
                  </div>
                </div>
                <button
                  onClick={() => { setTwoFAEnabled(e => !e); toast.success(twoFAEnabled ? '2FA disabled' : '2FA enabled — Use any TOTP app'); }}
                  className={`relative w-12 h-6 rounded-full transition-all ${twoFAEnabled ? 'bg-emerald-500' : 'bg-white/20'}`}
                >
              ...
              <div className="mt-4 flex items-start gap-3 p-3 rounded-xl border border-amber-500/20" style={{ background: 'rgba(245,158,11,0.08)' }}>
                ...
                <p className="text-xs text-slate-400">Sessions from unrecognized devices require email verification. Last login: Today at 10:23 AM from Chrome/Windows</p>
              </div>
            </GlassCard>

            <GlassCard>
              <h2 className="text-base font-bold text-white mb-4" style={{ fontFamily: 'Geist, sans-serif' }}>Danger Zone</h2>
              <div className="space-y-3">
        ...
        {tab === 'preferences' && (
          <motion.div key="preferences" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <GlassCard>
              <h2 className="text-base font-bold text-white mb-4" style={{ fontFamily: 'Geist, sans-serif' }}>App Preferences</h2>
              <div className="space-y-4">
                {[
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between py-3 border-b border-white/5 last:border-0">
                      <div>
                        <p className="text-sm font-medium text-white" style={{ fontFamily: 'Inter, sans-serif' }}>{pref.label}</p>
                        <p className="text-xs text-slate-500">{pref.desc}</p>
                      </div>
                      <button onClick={() => setOn(!on)} className={`relative w-12 h-6 rounded-full transition-all ${on ? 'bg-purple-500' : 'bg-white/20'}`}>
