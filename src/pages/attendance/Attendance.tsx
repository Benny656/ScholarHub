        ...
        <div className="flex items-center gap-1 mb-6 overflow-x-auto pb-1">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium flex-shrink-0 transition-all ${tab === t.id ? 'text-white' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          ...
          {tab === 'overview' && (
            <motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-5">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { label: 'Total Classes', value: summary.total, color: 'text-white' },
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div className="flex-1">
                      <div className="text-center mb-3">
                        <p className="text-4xl font-black text-white">{summary.percentage}%</p>
                      ...
                      {pieData.map(d => (
                        <div key={d.name} className="flex items-center justify-between text-sm mb-1">
                          <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full" style={{ background: d.fill }} /><span className="text-slate-400">{d.name}</span></div>
              ...
              <GlassCard tint="blue">
                ...
                {!qrData ? (
                  ...
                  <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center">
                    ...
                    <div className="w-48 h-48 mx-auto mb-4 rounded-2xl p-4 bg-white flex items-center justify-center">
                      <div className="grid grid-cols-8 gap-0.5 w-full h-full">
                        {Array.from({ length: 64 }, (_, i) => (
                          <div key={i} className="rounded-sm" style={{ background: Math.random() > 0.4 ? '#1a1a1a' : 'white' }} />
                        ))}
                      </div>
                    </div>
                    ...
                    <p className="text-xs text-slate-400 mb-4" style={{ fontFamily: 'JetBrains Mono, monospace' }}>Session: {qrData.sessionId}</p>
          ...
          {tab === 'scanner' && (
            <motion.div key="scanner" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="max-w-lg mx-auto">
              <GlassCard tint="blue">
                ...
                <div className="rounded-2xl overflow-hidden border border-white/10 aspect-square flex items-center justify-center mb-4" style={{ background: 'rgba(0,0,0,0.5)' }}>
                  <div className="relative w-48 h-48">
                    <div className="absolute inset-0 border-2 border-purple-500/60 rounded-xl" />
                    <div className="absolute top-0 left-0 w-6 h-6 border-t-4 border-l-4 border-purple-400 rounded-tl-xl" />
                    <div className="absolute top-0 right-0 w-6 h-6 border-t-4 border-r-4 border-purple-400 rounded-tr-xl" />
                    <div className="absolute bottom-0 left-0 w-6 h-6 border-b-4 border-l-4 border-purple-400 rounded-bl-xl" />
                    <div className="absolute bottom-0 right-0 w-6 h-6 border-b-4 border-r-4 border-purple-400 rounded-br-xl" />
                    <motion.div className="absolute left-0 right-0 h-0.5 bg-purple-400/60"
                      animate={{ top: ['10%', '90%', '10%'] }}
                      transition={{ duration: 2, repeat: Infinity, ease: 'linear' }}
                    />
                    <div className="absolute inset-0 flex items-center justify-center">
                      <p className="text-xs text-slate-400 text-center">Camera placeholder<br />(In production: use QR scanner)</p>
                    </div>
                  </div>
                </div>
            ...
            <motion.div key="reports" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <GlassCard padding="p-0">
                <div className="p-5 border-b border-white/5">
                  <h2 className="text-lg font-bold text-white">Student Attendance Report</h2>
                  <p className="text-sm text-slate-400">Full-Stack Web Development — Class Report</p>
                </div>
                        ...
                        <tr key={i} className="border-b border-white/5 hover:bg-white/3 transition-colors">
                          <td className="px-5 py-3.5">
                            <div className="flex items-center gap-2.5">
                              ...
                              <span className="font-medium text-white">{r.studentName}</span>
                            </div>
                          </td>
                          <td className="px-5 py-3.5 text-slate-300">{r.total}</td>
                          <td className="px-5 py-3.5 text-slate-300">{r.present}</td>
                          <td className="px-5 py-3.5">
            ...
            <motion.div key="insights" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="max-w-2xl mx-auto space-y-4">
              <GlassCard tint="purple">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h2 className="text-lg font-bold text-white" style={{ fontFamily: 'Geist, sans-serif' }}>AI Attendance Insights</h2>
                </div>
                {insights.length === 0 ? (
                  <div className="text-center py-8">
                    <p className="text-slate-400 mb-4 text-sm">Get AI-powered insights about your attendance patterns</p>
                  ...
                  <div className="space-y-3">
                    {insights.map((ins, i) => (
                      <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}
                        ...
                        <p className="text-sm font-semibold text-white mb-1" style={{ fontFamily: 'Inter, sans-serif' }}>{ins.insight}</p>
                        <p className="text-xs text-slate-400">{ins.recommendation}</p>
                      </motion.div>
                    ))}
