...
import {
  ...
  Circle, Square, Eraser, Minus, X, Send, ChevronRight, ChevronLeft, Moon, Sun
} from 'lucide-react';
    ...
    >
      {p.isCameraOff ? (
        <div className={`absolute inset-0 flex items-center justify-center bg-gradient-to-br ${colors[index % colors.length]} opacity-30`}>
          <div className="w-16 h-16 rounded-full bg-white/20 flex items-center justify-center text-2xl font-bold text-on-surface">
            {p.name[0]}
          </div>
        </div>
      ) : (
        <div className={`absolute inset-0 bg-gradient-to-br ${colors[index % colors.length]} opacity-20`}>
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-16 h-16 rounded-full bg-on-surface/10 flex items-center justify-center text-2xl font-bold text-on-surface">
              {p.name[0]}
            </div>
          </div>
        </div>
      ...
      {/* Host badge */}
      {p.role === 'host' && (
        <div className="absolute top-2 left-2 text-xs font-bold px-1.5 py-0.5 rounded-md text-on-surface" style={{ background: 'rgba(139,92,246,0.8)' }}>Host</div>
      )}
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 p-2 border-b border-outline-variant/10">
        ...
        <div className="w-px h-5 bg-on-surface/10 mx-1" />
        <button onClick={clearCanvas} className="p-2 rounded-lg text-on-surface-variant hover:text-red-400 hover:bg-red-500/10 transition-all"><Eraser size={16} /></button>
      </div>
      <canvas
        ref={canvasRef}
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-outline-variant/10 bg-white/95 dark:bg-neutral-950/95 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-semibold text-on-surface" style={{ fontFamily: 'Geist, sans-serif' }}>Advanced React Patterns — Live</span>
        ...
        <div className="flex items-center justify-center gap-3 py-4 px-4 border-t border-outline-variant/10 bg-white/95 dark:bg-neutral-950/95 backdrop-blur-md">
          ...
          <button className="w-12 h-12 rounded-2xl flex items-center justify-center bg-white/8 text-on-surface hover:bg-white/12 border border-outline-variant/20 transition-all">
            <MonitorPlay size={20} />
          </button>
          <motion.button
          ...
          <button
            ...
            className="w-12 h-12 rounded-2xl flex items-center justify-center bg-white/8 text-on-surface hover:bg-white/12 border border-outline-variant/20 transition-all"
          >
            <Grid size={20} />
          </button>
          <div className="w-px h-8 bg-on-surface/10" />
          <button className="flex items-center gap-2 px-5 py-3 rounded-2xl text-sm font-semibold text-on-surface transition-all" style={{ background: 'linear-gradient(135deg, #EF4444, #DC2626)', boxShadow: '0 4px 16px rgba(239,68,68,0.3)' }}>
        ...
        {chatOpen && (
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-3">
              {messages.map((m, i) => (
                <div key={i}>
                  <p className={`text-xs font-semibold mb-0.5 ${m.isHost ? 'text-[#2563EB]' : 'text-[#2563EB]'}`}>{m.sender}</p>
            ...
            {/* Raised hands */}
            {handRaised && (
              <div className="px-3 py-2 border-t border-outline-variant/10 border-b" style={{ background: 'rgba(245,158,11,0.1)' }}>
                <p className="text-xs font-semibold text-amber-400 mb-1">✋ Raised Hands</p>
                <p className="text-xs text-on-surface-variant">Alex Johnson</p>
              </div>
            )}
          ...
          >
            ...
            {['Room A — Frontend', 'Room B — Backend', 'Room C — Design'].map((r, i) => (
              <div key={i} className="flex items-center justify-between px-4 py-3 border-b border-outline-variant/10 hover:bg-on-surface/5 transition-colors">
                <div>
                  <p className="text-sm font-medium text-on-surface">{r}</p>
                  <p className="text-xs text-on-surface-variant">{[2, 3, 1][i]} participants</p>
                </div>
            ...
            <div className="p-3">
              <button className="w-full py-2 rounded-xl text-xs font-semibold text-on-surface transition-all" style={{ background: 'linear-gradient(135deg, #2563EB, #3B82F6)' }}>
                Create Room
              </button>
            </div>
          </motion.div>
